	...
	testCases := []*testCase{
		{
			...
			setupMocks: func(c Config, m *mocksWrapper, testCase *testCase) {
				...
				m.State.On("GetNonce", context.Background(), *txArgs.From, blockNumOne.Uint64(), m.DbTx).Return(nonce, nil).Once()
				m.State.
					On("ProcessUnsignedTransaction", context.Background(), txMatchBy, *txArgs.From, blockNumOne.Uint64(), false, m.DbTx).
		...
		},
		{
			...
			setupMocks: func(c Config, m *mocksWrapper, testCase *testCase) {
				...
				m.State.
					On("ProcessUnsignedTransaction", context.Background(), txMatchBy, *txArgs.From, blockNumTen.Uint64(), false, m.DbTx).
					Return(&runtime.ExecutionResult{ReturnValue: testCase.expectedResult}, nil).
					Once()
			},
		},
		...
		},
		{
			...
			setupMocks: func(c Config, m *mocksWrapper, testCase *testCase) {
				...
				m.State.
					On("ProcessUnsignedTransaction", context.Background(), txMatchBy, common.HexToAddress(c.DefaultSenderAddress), blockNumOne.Uint64(), false, m.DbTx).
					Return(&runtime.ExecutionResult{ReturnValue: testCase.expectedResult}, nil).
					Once()
			},
		},
